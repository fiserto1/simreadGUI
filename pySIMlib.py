...
class pySIMlib:
    ...
    def sendAPDU(self, command, checkSW=False, refSW=""):
        ...
        for i in range(5):
            ...
            self.serialport.write(chr(s).encode())
        ...
        if checkSW:
            if sw != refSW:
                raise RuntimeError("Status words do not match. Result: %s, Expected: %s" % (sw, refSW))
